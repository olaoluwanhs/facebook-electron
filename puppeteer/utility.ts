...
export const launchBrowser = async (
  data: browserOptions = { autoCloseAlert: false, disableNotifications: true }
  ...
  const browser = await puppeteer.launch({
    headless: false,
    args: [
      `${data.disableNotifications && "--disable-notifications"}`,
      "--no-sandbox",
      "--trace-warnings",
      `--window-size=1404,1024`,
    ],
    // executablePath: 'C:/Program Files/Google/Chrome/Application/chrome.exe',
  ...
  origins.forEach((origin) => {
    context.overridePermissions(`https://${origin}`, [
      "geolocation",
      "notifications",
    ]);
  });
  ...
  if (data.autoCloseAlert) {
    page.on("dialog", async (dialog) => {
      console.log(
        `Dialog of type '${dialog.type()}' detected with message: '${dialog.message()}'`
      );
...
export const loginToAccount = async (
  ...
  try {
    ...
    const cookiesPath = resolve(
      __dirname,
      "cookies",
      `${loginInfo.email.replace(".com", "")}-cookies.json`
    );
    ...
    const HomeButtonFirstCheck = await page.waitForSelector(
      '*[aria-label="Home"]',
      { timeout: 60000 }
    );
    ...
    try {
      ...
      if (
        pageContent.includes(
          `Save your password now to make logging in even easier.`
        )
      ) {
    ...
    const HomeButton = await page.waitForSelector('*[aria-label="Home"]', {
      timeout: 60000,
    });
    ...
    try {
      ...
      if (await HomeButton?.isVisible()) {
        const newcookies = await page.cookies();
        fs.writeFileSync(
          `/cookies/${loginInfo.email}-cookies.json`,
  ...
  try {
    ...
    const rect = await page.evaluate((postSomething) => {
      const { top, left, height, width } =
        postSomething.getBoundingClientRect();
    ...
    await page.waitForFunction(
      () => {
        return !document.querySelector('*[aria-label="Post"]');
      },
      { timeout: 30 * 60 * 1000 }
    );
...
export const getMyProfiles = async (
  page: Page,
  returnHandle: boolean = true
): Promise<[string[], ElementHandle<Element>[]] | [string[], null]> => {
  try {
    await Promise.all([
    ...
    await delayedResolve();
    const profileIcon = await page.waitForSelector(
      '*[aria-label="Your profile"]'
    );
    await profileIcon?.click();

    const seeAllProfiles = await page.waitForSelector(
      '*[aria-label="See all profiles"]'
    );
    await delayedResolve();
    ...
    const pages = await page.$$(
      `div[style="transform: translateX(0%) translateZ(1px);"] div[role="list"] div[role="listitem"]`
    );
    ...
    for (const p in pages) {
      const text: string = (await (
        await pages[p].getProperty("innerText")
      ).jsonValue()) as string;
      if (!text.includes("Create new profile or Page")) {
        pageNames.push(text.split("\n")[0]);
        pageIndex.push(Number(p));
      }
    }

    return returnHandle
      ? [pageNames, pages.filter((_, i) => pageIndex.includes(i))]
      : [pageNames, null];
  } catch (error) {
    return [[], null];
  }
...
export const SwitchProfiles = async (page: Page, ProfileName: string) => {
  ...
  // await moreBtn.click()
  if (pages[1]) {
    await Promise.all([pages[1][divIndex].click(), page.waitForNavigation()]);
  }
  ...
  const profileTags = await page.$$(
    '*[href*="https://www.facebook.com/profile.php"]'
  );
  for (const h of profileTags) {
    const name = (await (
      await h.getProperty("innerText")
    ).jsonValue()) as string;
    if (
      name.toLowerCase().trim().includes(ProfileName.toLocaleLowerCase().trim())
    ) {
      changedProfile = true;
